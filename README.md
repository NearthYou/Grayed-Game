...
첫 구현은 타일을 놓을 때마다 Prefab을 생성했습니다. 기능은 동작했지만 맵이 커지자 같은 환경의 Unity Profiler에서 Editor 메인 스레드의 한 프레임 처리 시간이 1084.8ms까지 늘었습니다. 편집 중에는 좌표만 저장하고 Gizmo로 표시하며 실제 오브젝트는 플레이 시작 시 한 번에 생성하도록 바꿔 52.7ms로 줄였습니다.
...
문제: 타일마다 Prefab을 즉시 생성해 편집이 느려졌고, 에디터 좌표와 실제 그리드 좌표가 어긋나 오브젝트가 겹쳤습니다. 에디터 전용 코드가 빌드에 포함되는 문제도 함께 있었습니다.

해결:
- 편집 중에는 좌표만 저장하고 Gizmo로 표시한 뒤 플레이 시작 시 실제 오브젝트 생성
