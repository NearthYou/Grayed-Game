...
2024년 working draft의 미완성 5개 chapter, placeholder story와 GameOver 가제는 현재 설계로 사용하지 않습니다. 이 문서는 실제 README와 구현된 CH3, Dancepace 범위만 설명합니다.
...
- Dancepace rhythm mini game
- Yarn Spinner dialogue와 event trigger
- 지역을 연결하는 teleporter system
...
editor에서 배치한 object와 play 중 `BuildingObjectFactory`가 만드는 building은 같은 data contract를 사용합니다. 기획자가 배치한 field와 runtime building이 다른 규칙으로 움직이는 문제를 줄이기 위한 구조입니다.

player는 resource를 수집해 building을 만들고 production item을 얻습니다. inventory, hotbar, tooltip과 sound feedback은 이 loop를 화면에 연결합니다.

## Dancepace rhythm mini game

Dancepace는 key input, wave progression, time limit와 result panel을 가진 rhythm mini game입니다.

wave와 game setting은 ScriptableObject에 두고 string table과 연결합니다. rehearsal, answer character, audience effect와 sound는 game flow state에 따라 나뉩니다.

input 판정과 presentation을 한 class에 몰지 않고 manager, data, character, effect와 UI 단위로 나눈 것이 현재 source의 경계입니다.

## 현재 구현 범위

- Unity 2022.3.62f2 기반 PC project
- chapter별로 다른 장르와 system을 결합하는 구조
- CH3 field editor와 runtime grid
...
게임 전체 story와 모든 chapter가 완성됐다고 주장하지 않습니다. README의 개발 기간과 공개 build는 2026년 1월까지 확인된 project record를 기준으로 합니다.
